...
```json
{
    "repositories": [
        {
            "type": "composer",
            "url": "https://<licensing-host>/wp-json/skl/v1/"
        }
    ],
    "require": {
...
If this is an already-installed project (adding a license after the fact, not a fresh install),
regenerate the root `.env` first — Docker Compose reads `COMPOSER_AUTH` from `.env`, not directly
from `config/environment/.env.secret`:
...
- **Same 401-style failure right after editing `.env.secret` on an already-installed project.**
  You likely skipped `make env` — the root `.env` file Docker Compose actually reads is only
  regenerated by `make env`/`make install`, not automatically when `.env.secret` changes. See
  step 5 above.
